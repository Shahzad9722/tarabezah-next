...
export const ElementProperties: React.FC = () => {
  const { activeFloorplan, selectedElementId, elementLibrary, updateElement, deleteElement } = useFloorplan();

  const [name, setName] = useState('');
  const [minCapacity, setMinCapacity] = useState('');
  const [maxCapacity, setMaxCapacity] = useState('');

  const selectedElement = activeFloorplan?.elements.find((el) => el.localId === selectedElementId);
  const libraryItem = selectedElement ? elementLibrary.find((item) => item.id === selectedElement.libraryItemId) : null;
      ...
      <CardHeader className='flex flex-row items-center align justify-between pb-2'>
        <CardTitle>
          <span className=' text-xs pb-1'>
            <img src={libraryItem.elementImageUrl} alt={libraryItem.name} className='max-w-[50px] max-h-[50px]' />
          </span>
        </CardTitle>
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className='space-y-4'>
          <div className='space-y-2'>
            ...
            <Input id='name' value={name} onChange={(e) => setName(e.target.value)} placeholder={libraryItem.name} />
          </div>
