...
export default function Navigation({ onPublish, disabled }: { onPublish?: any, disabled?: boolean }) {
  ...
  const linkBaseClasses =
    'flex items-center gap-2 md:gap-4 rounded-lg text-md font-normal px-2 py-2 md:px-4 md:py-4 whitespace-nowrap';
  const activeLinkClasses = 'bg-color-222036 text-white';
  const inactiveLinkClasses = 'text-color-BE963C hover:bg-color-222036';

  return (
    <div className='w-full flex flex-col md:flex-row md:items-center justify-between my-2 px-4'>
      <div className='flex items-center overflow-x-auto mb-4 md:mb-0 md:overflow-x-visible gap-x-2'>
        <Link
          href='/'
          className={`${linkBaseClasses} ${pathname === '/' ? activeLinkClasses : inactiveLinkClasses
            }`}
        >
        ...
        <Link
          ...
          className={`${linkBaseClasses} ${pathname === '/table-combination' ? activeLinkClasses : inactiveLinkClasses
            }`}
        >
