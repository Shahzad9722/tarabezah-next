...
export default function AddReservation({
  ...
  const {
    isPending: fetchingEntities,
    data: entities = { sources: [], tags: [], shifts: [], tableTypes: [] },
  } = useQuery({
    queryKey: ["entities"],
    queryFn: async () => {
      const res = await fetch(`/api/reservation/form/entities`);
  ...
  const { mutateAsync: addReservation, isPending: submittingReservationForm } =
    useMutation({
      mutationFn: (data: any) => {
  ...
  const { mutateAsync: addWalkin, isPending: submittingWalkinForm } =
    useMutation({
      mutationFn: (data: any) => {
  ...
  const checkForm = async (e: any) => {
    ...
    try {
      ...
      if (!walkIn) {
        ...
        const isValid = errorFields.every(
  ...
  useEffect(() => {
    if (Object.keys(reservationForm?.formState?.errors).length) {
      const step = (walkIn ? stepsWalkIn : steps)
        ...
        .find((s) =>
          s.fields.includes(Object.keys(reservationForm?.formState?.errors)[0])
        );
      if (step) {
    ...
    <div className="md:h-screen flex flex-col md:flex-row bg-color-121020 bg-[linear-gradient(119.26deg,_rgba(18,_17,_32,_0.23)_45.47%,_rgba(185,_136,_88,_0.23)_105.35%)] shadow-lg w-full min-h-screen">
      ...
      <div className="w-full flex-1 p-6 overflow-x-auto">
          ...
          <>
            ...
            {!selectedClient.id && (
              <p className="text-red-500 mb-6">
                {reservationForm?.formState?.errors?.clientId?.message}
              </p>
            )}
          </>
        )}
        ...
        {showAddNewClient && (
          <Form {...guestForm}>
            <form
              onSubmit={guestForm.handleSubmit(createGuest)}
              className="space-y-4"
            >
